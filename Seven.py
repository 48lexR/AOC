...
class Dir:
    def __init__(self, head, name: str) -> None:
        self.head = head
        self.name = name
        self.files = []
        self.dirs = []
...
def get_smallest_possible_dir(required_space: int, current_best: Dir, current_dir: Dir):
    for dir in current_dir.dirs:
        current_best = get_smallest_possible_dir(
            required_space, current_best, dir)
...
if __name__ == "__main__":
    data = open(r"inputs/Seven.txt", 'r')
    inputdata = np.asarray([line.strip() for line in data])
    root = parseInput(inputdata)
    set_dir_sizes(root)
    ############################## PART 1 ##############################
    print(
        f"the sum of the directories with size <= 100000 is {get_dir_sizes(0, root)}")
    ############################## PART 2 ##############################
    required_space = root.size - 40000000
    smallest_possible_dir = get_smallest_possible_dir(
        required_space, root, root)
    print(
        f"the smallest possible directory is {smallest_possible_dir} with size {smallest_possible_dir.size}")
