...
class Monkey:
    def __init__(self, items, operationOperator, operationValue, testDivisibleValue: int, monkeyThrowToIfTrue: int, monkeyThrowToIfFalse: int):
        self.items = items
    ...
    def __inspectItem(self):
        ...
        if self.operationValue == "old":
            self.items[0].worryLevel = ops[self.operationOperator](
                self.items[0].worryLevel, self.items[0].worryLevel)
        else:
            self.items[0].worryLevel = ops[self.operationOperator](
                self.items[0].worryLevel, self.operationValue)
        if self.dontWorry:
            if self.items[0].worryLevel > self.commonDenominator:
                self.items[0].worryLevel = int(
                    self.items[0].worryLevel % self.commonDenominator)
        else:
...
def gatherAllMonkey(myMonkeyCircus):
    file1 = open('inputs/Eleven.txt', 'r')
    ...
    for line in Lines:
        if "Monkey " in line:
            ...
            monkeyOperatorValue = line.split("Operation: new = old ")[
                1].split(" ")[1].strip()
            if monkeyOperatorValue != "old":
                monkeyOperatorValue = int(monkeyOperatorValue)
        elif "Test: divisible by " in line:
            testDivisibleValue = int(line.split(
                "Test: divisible by ")[1].strip())
        elif "If true: throw to monkey " in line:
            monkeyThrowToIfTrue = int(line.split(
                "If true: throw to monkey ")[1].strip())
        elif "If false: throw to monkey " in line:
            monkeyThrowToIfFalse = int(line.split(
                "If false: throw to monkey ")[1].strip())
            myMonkeyCircus.addNewMonkey(Monkey(
                items, monkeyOperator, monkeyOperatorValue, testDivisibleValue, monkeyThrowToIfTrue, monkeyThrowToIfFalse))
    ...
    for x in range(0, roundCount):
        for i in range(len(monkeyCircus.monkeys)):
            for p in range(len(monkeyCircus.monkeys[i].items)):
                ...
                monkeyCircus.monkeys[throwElement[0]
                                     ].catchItem(throwElement[1])
    rankingInspectionCount = []
    for i in range(len(monkeyCircus.monkeys)):
        rankingInspectionCount.append(
            monkeyCircus.monkeys[i].getInspectionCount())
